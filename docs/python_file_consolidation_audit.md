...
当前结论是：`backend/graph/checkpoint.py` 已确认没有仓库内引用，并已删除；当前未发现新的可无条件删除核心 Python 文件。
...
Current update: backend/mcp_client has been removed after confirming there are no in-repo imports.
Current update: backend/graph/checkpoint.py has been removed after confirming snapshot_state has no in-repo imports.
...
| `backend/tool_gateway.py` | 当前真实 ToolGateway | `backend/harness/graph_runner.py`、`backend/harness/agent_service.py`、`backend/tests/test_tool_gateway.py` | 保留；不要删除真实实现 |
...
该候选项已完成删除；后续审计继续以实际 import 与测试结果为准。
...
4. 已完成：确认仓库内没有使用 `backend/mcp_client/client_manager.py` 和 `backend/mcp_client/tool_invoker.py`，并删除 `backend/mcp_client/` 兼容 facade。
...
3. 不创建 backend/rag/、backend/skills/、backend/database/、frontend/src/stores/；frontend/src/api/researchAgent.ts 已作为请求层存在。

执行步骤：
1. 重新运行 git status --short，确认工作区状态。
2. 重新运行 rg 检查以下文件的 import：
   - backend/harness/runtime.py
3. backend/graph/checkpoint.py 已删除；无需继续作为候选项跟踪。
4. backend/mcp_client 兼容 facade 已删除；当前结构入口是 backend/tool_gateway.py 与 Harness service/graph runner。
